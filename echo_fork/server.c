...
void handle_client(int cd, struct sockaddr_in client_addr) {
...
int main(int argc, char* argv[]){
	...
	while(1) {
		...
		if(pid == 0) {
			// figlio
			close(ld); // figlio chiude listener 
			handle_client(cd, client_addr);
		}
		
		close(cd); // padre chiude client
	}
}
